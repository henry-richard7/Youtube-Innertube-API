...
class YoutubeInnerApi:
    ...
    def get_video_metrics(video_id):
        ...
        try:
            ...
            post_create_date = response['contents']['twoColumnWatchNextResults']['results']['results']['contents'][0]['videoPrimaryInfoRenderer']['dateText']['simpleText']
            is_premium = False
        except:
            view_count = None
            likes_count = None
        ...
        try:
            ...
            result['comment_count'] = int(response2['onResponseReceivedEndpoints'][0]['reloadContinuationItemsCommand']['continuationItems'][0]['commentsHeaderRenderer']['countText']['runs'][0]['text'].replace(',',''))
    ...
    def get_playlist(self,playlistID):
        ...
        while True:
                ...
                if not continuation:
                    items = playlist_response['contents']['twoColumnBrowseResultsRenderer']['tabs'][0]['tabRenderer']['content']\
                    ...
                    for playlistItem in items:
                            if playlistItem.get('playlistVideoRenderer'):
                                results.append({
                                    'video_id': playlistItem['playlistVideoRenderer']['videoId'],
                    ...
                    try:
                        playlist_payload['continuation'] = playlist_response['contents']['twoColumnBrowseResultsRenderer']['tabs'][0]\
                        ...
                        try:
                            playlist_payload['continuation'] = playlist_response['onResponseReceivedActions'][0]['appendContinuationItemsAction']['continuationItems'][-1]\
    ...
    def get_channel(self,channel_id:str):
        ...
        sub_count = channel_response['header']['c4TabbedHeaderRenderer']['subscriberCountText']['simpleText'].replace(" subscribers","")
        if "K" in sub_count:
        ...
        views_count = about_response['contents']['twoColumnBrowseResultsRenderer']['tabs'][-2]\
            ['tabRenderer']['content']['sectionListRenderer']['contents'][-1]\
            ['itemSectionRenderer']['contents'][-1]['channelAboutFullMetadataRenderer']\
                ['viewCountText'].get('simpleText').split()[0].replace(",","")

        joined_date = about_response['contents']['twoColumnBrowseResultsRenderer']['tabs'][-2]\
            ['tabRenderer']['content']['sectionListRenderer']['contents'][-1]\
                ['itemSectionRenderer']['contents'][-1]['channelAboutFullMetadataRenderer']\
                    ['joinedDateText']['runs'][-1]['text']

        country = about_response['contents']['twoColumnBrowseResultsRenderer']['tabs'][-2]\
            ['tabRenderer']['content']['sectionListRenderer']['contents'][-1]\
                ['itemSectionRenderer']['contents'][-1]['channelAboutFullMetadataRenderer']\
                    ['country']['simpleText']
        ...
        bio = about_response['contents']['twoColumnBrowseResultsRenderer']['tabs'][-2]\
            ['tabRenderer']['content']['sectionListRenderer']['contents'][-1]\
                ['itemSectionRenderer']['contents'][-1]['channelAboutFullMetadataRenderer'].get('artistBio')
